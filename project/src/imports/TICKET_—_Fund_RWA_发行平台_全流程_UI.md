...
| Redemption Record | **沿用 Bond Redemption 逻辑并切换为 Fund 文案** | 基金赎回到账记录 |
| Distribution Record | **新增并参考 Bond Coupon Payment 逻辑** | 基金分配 / 收益领取记录（含 Accept 入口） |
...
> 参考 `TP_Coupon_Distribution_Process.mp4` 末尾的 User 页面。Fund 版本将 Bond 的 `Coupon Payment Record` 更名为 `Distribution Record`。
...
对应视频中的 `Open For Coupon Payment`，Fund 版本文案统一改为：
...
参考视频中的 `Accept Coupon Payment`，Fund 版本改名为：
